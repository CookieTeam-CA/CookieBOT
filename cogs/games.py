...
async def bot_rps(choice):
    ...
    if (choice == "Stein" and bot_choice == "Schere") or \
         (choice == "Papier" and bot_choice == "Stein") or \
...
async def select_winner(user_selection1, user_selection2):
    ...
    if user_selection1 == user_selection2:
        return draw
    if (user_selection1 == "Stein" and user_selection2 == "Schere") or \
         (user_selection1 == "Papier" and user_selection2 == "Stein") or \
         (user_selection1 == "Schere" and user_selection2 == "Papier"):
        return
    return
...
class RPSView(discord.ui.View): # code multiplayer rps
    ...
    async def button_callback2(self, button, interaction):
        if interaction.user != self.author:
            await interaction.response.send_message(f"Nur {self.author.mention} kann diesen Button drücken.",
                                                    ephemeral=True)
            return
...
class BotRPSView(discord.ui.View):
    ...
    async def button_callback1(self, button, interaction):
        if interaction.user != self.author:
            await interaction.response.send_message(f"Nur {self.author.mention} kann diesen Button drücken.", ephemeral=True)
            return
