...
async def fetch_meme_msg_ids(db_path: str, message_id: int) -> list[int]:
    async with aiosqlite.connect(db_path) as db:
        async with db.execute("SELECT 1 FROM memes WHERE msg_id = ?",(message_id,)) as cursor:
...
class MemeVoting(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if message.attachments:
            async with aiosqlite.connect(self.database) as db:
                await db.execute("INSERT INTO memes (user_id, msg_id, meme_url, meme_txt) VALUES (?, ?, ?, ?)", (message.author.id, message.id, message.attachments[0].url, message.content))
                await db.commit()
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if payload.emoji.name in ("⬆️", "⬇️"):
            ...
            async with aiosqlite.connect(self.database) as db:
                try:
                    await db.execute("INSERT INTO meme_votes (msg_id, user_id, vote) VALUES (?, ?, ?)", (payload.message_id, payload.user_id, vote))
                ...
                await db.execute("UPDATE memes SET votes = votes + ? WHERE msg_id = ?", (vote, payload.message_id))
                await db.commit()
    ...
    async def on_raw_reaction_remove(self, payload):
        ...
        if payload.emoji.name in ("⬆️", "⬇️"):
            ...
            async with aiosqlite.connect(self.database) as db:
                await db.execute("DELETE FROM meme_votes WHERE msg_id = ? AND user_id = ?", (payload.message_id, payload.user_id))
